...
import { renderWithProviders, setupWalletMocks, userEvent } from '../../test/utils'
...
describe('WalletConnection', () => {
  ...
  describe('when wallet is not connected', () => {
    ...
    it('should render full layout when showTitle and showDescription are true', () => {
      ...
      expect(screen.getByText(/connect your wallet to begin/i)).toBeInTheDocument()
  ...
  describe('variant props', () => {
    it('should apply correct classes for default variant', () => {
      ...
      expect(button).toHaveClass('border-white/20', 'backdrop-blur-sm', 'text-white')
    })
  ...
  describe('conditional rendering', () => {
    it('should render only title when showTitle is true', () => {
      renderWithProviders(<WalletConnection showTitle={true} showDescription={false} />)
      
      expect(screen.getByText(/connect your wallet to begin/i)).toBeInTheDocument()
      expect(screen.queryByText(/any descriptive copy/i)).not.toBeInTheDocument()
    })

    it('should render only description when showDescription is true', () => {
      renderWithProviders(<WalletConnection showTitle={false} showDescription={true} />)
      
      expect(screen.queryByText(/connect your wallet to begin/i)).not.toBeInTheDocument()
      expect(screen.getByText(/any descriptive copy/i)).toBeInTheDocument()
    })

    it('should render simple button when both title and description are false', () => {
      renderWithProviders(<WalletConnection showTitle={false} showDescription={false} />)
