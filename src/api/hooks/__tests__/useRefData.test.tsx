...
describe("useRefData", () => {
  ...
  it("should refetch data when refetch is called", async () => {
    ...
    mockedGet.mockResolvedValueOnce(mockData1).mockResolvedValueOnce(mockData2);
  ...
  it("should cache data correctly", async () => {
    ...
    const { result: result2 } = renderHook(() => useRefData(), {
      wrapper: sharedWrapper,
    });
  ...
  it("should handle different response structures", async () => {
    ...
    expect(
      (result.current.data as { config: { timeout: number } }).config.timeout,
    ).toBe(5000);
  });
});
