...
export function addPendingTransaction(transaction: Omit<PendingTransaction, 'status' | 'attempts' | 'maxAttempts'>): void {
  try {
    const transactions = getPendingTransactions()
    
    const existingIndex = transactions.findIndex(tx => tx.id === transaction.id)
    if (existingIndex !== -1) {
      console.warn(`Transaction ${transaction.id} already exists, not adding duplicate`)
      return
    }
...
export function updateTransactionStatus(id: string, status: 'pending' | 'complete'): void {
  try {
    const transactions = getPendingTransactions()
...
export function cleanupCompletedTransactions(): void {
  try {
    ...
    if (filtered.length !== transactions.length) {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(filtered))
      console.log(`Cleaned up ${transactions.length - filtered.length} old completed transactions`)
    }
  } catch (error) {
